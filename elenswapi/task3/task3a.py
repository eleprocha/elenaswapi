...
def search(world,**kwargs):
    ...
    if any({i:j for i,j in name_uid_dict.items() if kwargs['char'].replace("'","").upper() in i.upper()}):
        dict = {}
        dict['name'] = [i for i in name_uid_dict.keys() if kwargs['char'].replace("'","").upper() in i.upper()][0]
        ...
        print('\n\ncached:'+datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f'))
    else:
